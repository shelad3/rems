...
class _MpesaPaymentScreenState extends ConsumerState<MpesaPaymentScreen> {
  ...
  Future<void> _pay() async {
    ...
    try {
      final checkoutId = await _mpesaService.stkPush(
        ...
        phone: phone,
        accountReference: 'RENT-${widget.propertyId.substring(0, 6)}',
      );
  ...
  void _listenForServerConfirm(String checkoutId) {
    ...
    _confirmTimeout = Timer(const Duration(seconds: 75), () {
      if (!_settled && mounted) {
        setState(() {
          _statusMessage = 'Payment still unconfirmed. Check your statement and retry.';
          _loading = false;
        });
      }
    });
    _requestSub = ref
      ...
      if (request != null && request.status == 'completed') {
        ...
        setState(() {
          _success = true;
          _statusMessage = 'Payment successful! Receipt: ${request.receipt}';
          _loading = false;
        });
      } else if (request != null && (request.status == 'failed' || request.status == 'mismatch')) {
        _settled = true;
        _confirmTimeout?.cancel();
        setState(() {
  ...
  Future<void> _confirmFromServer(String checkoutId, String phone) async {
    try {
      ...
      if (response.statusCode == 200 && body['ok'] == true && body['status'] == 'completed') {
        setState(() {
          _success = true;
          _statusMessage = 'Payment successful! Receipt: ${body['receipt']}';
          _loading = false;
        });
        ...
        setState(() {
          _statusMessage = 'Payment confirmation failed: ${body['reason'] ?? 'Unknown'}';
          _loading = false;
        });
      }
    } on FormatException {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Amount Due', style: TextStyle(color: AppColors.textSecondary, fontSize: 14)),
                    const SizedBox(height: 4),
                    Text(Helpers.formatCurrency(widget.amount), style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
            const SizedBox(height: 24),
            const Text('M-Pesa Phone Number', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
            ...
            const SizedBox(height: 8),
            const Text('Enter the M-Pesa registered phone number', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
            const SizedBox(height: 24),
            if (_statusMessage != null)
              Container(
                ...
                decoration: BoxDecoration(
                  color: _success ? AppColors.success.withAlpha(20) : AppColors.error.withAlpha(20),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Row(
                  children: [
                    Icon(_success ? Icons.check_circle : Icons.info, color: _success ? AppColors.success : AppColors.error, size: 20),
                    const SizedBox(width: 8),
                    Expanded(child: Text(_statusMessage!, style: TextStyle(color: _success ? AppColors.success : AppColors.error, fontSize: 13))),
                  ],
                ),
              ),
            ...
            SizedBox(
              ...
              child: _success
                  ...
                  : ElevatedButton.icon(
                      ...
                      label: Text(_loading ? 'Processing...' : 'Pay with M-Pesa'),
                      style: ElevatedButton.styleFrom(backgroundColor: AppColors.success),
                    ),
            ),
          ],
